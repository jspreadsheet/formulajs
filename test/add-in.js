...
describe('Add-in', () => {
  it('EUROCONVERT', () => {
    expect(addIn.EUROCONVERT(1.2, 'DEM', 'EUR')).to.eq(0.61)
    ...
    expect(addIn.EUROCONVERT(0, 'DEM', 'EUR')).to.eq(0)
    expect(addIn.EUROCONVERT(0, 'EEK', 'FRF')).to.eq(0)
    ...
    expect(addIn.EUROCONVERT(1.2, 'DEM')).to.eq(error.na)
    expect(addIn.EUROCONVERT(1.2)).to.eq(error.na)
    expect(addIn.EUROCONVERT()).to.eq(error.na)

    Object.values(error).forEach((err) => {
      expect(addIn.EUROCONVERT(err, 'DEM', 'EUR')).to.eq(err)
      expect(addIn.EUROCONVERT(1.2, err, 'EUR')).to.eq(err)
      expect(addIn.EUROCONVERT(1.2, 'DEM', err)).to.eq(err)
      expect(addIn.EUROCONVERT(1.2, 'DEM', 'EUR', err)).to.eq(err)
      expect(addIn.EUROCONVERT(1.2, 'DEM', 'EUR', false, err)).to.eq(err)
    })
  })
})
